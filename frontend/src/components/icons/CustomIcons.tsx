...
export function FinSightLogo({ size = "large" }: FinSightLogoProps) {
  const theme = useTheme();
  const iconSize = size === "large" ? 40 : 28;
  const fontSize = size === "large" ? "h5" : "h6";

  // 다크 모드와 라이트 모드에 따라 색상 변경
  const textColor = theme.palette.mode === "dark" ? "white" : "black";
  const iconColor = theme.palette.mode === "dark" ? "white" : "black";

  return (
    <Box display="flex" alignItems="center" gap={1}>
      <AutoGraphIcon sx={{ fontSize: iconSize, color: iconColor }} />
      <Typography
        variant={fontSize}
